...
def main():
    ...
    if args.blackboard:
        ...
        from .protocol_def import P40_DEF

        if args.dry_run:
            print(f"[dry-run] Protocol: {P40_DEF.protocol_id}, stages: {[s.name for s in P40_DEF.stages]}")
            return

        client = make_client(protocol_id="p40_boyd_ooda", trace=getattr(args, 'trace', False), trace_path=__import__('pathlib').Path(args.trace_path) if getattr(args, 'trace_path', None) else None)
        config = {
            "client": client,
        ...
        orch = Orchestrator()
        bb = asyncio.run(orch.run(P40_DEF, args.question, agents, **config))

        print("\n" + "=" * 70)
        print("BOYD OODA RESULTS (blackboard)")
        print("=" * 70)
