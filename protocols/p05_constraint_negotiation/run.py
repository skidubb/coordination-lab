...
def main():
    ...
    parser.add_argument("--orchestration-model", default=ORCHESTRATION_MODEL, help="Model for constraint extraction")
    ...
    if args.blackboard:
        ...
        client = make_client(protocol_id="p05_constraint_negotiation", trace=getattr(args, 'trace', False), trace_path=Path(args.trace_path) if getattr(args, 'trace_path', None) else None)
        config = {
            "client": client,
