...
def main() -> None:
    ...
    if args.blackboard:
        ...
        from .protocol_def import P20_DEF

        if args.dry_run:
            print(f"[dry-run] Protocol: {P20_DEF.protocol_id}, stages: {[s.name for s in P20_DEF.stages]}")
            return

        client = make_client(protocol_id="p20_borda_count", trace=getattr(args, 'trace', False), trace_path=__import__('pathlib').Path(args.trace_path) if getattr(args, 'trace_path', None) else None)
        config = {
            "client": client,
        ...
        orch = Orchestrator()
        bb = asyncio.run(orch.run(P20_DEF, args.question, agents, **config))

        print("\n" + "=" * 70)
        print("BORDA COUNT RESULTS (blackboard)")
        print("=" * 70)
