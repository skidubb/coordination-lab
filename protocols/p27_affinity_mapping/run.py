...
def main() -> None:
    ...
    if args.blackboard:
        ...
        client = make_client(protocol_id="p27_affinity_mapping", trace=getattr(args, 'trace', False), trace_path=Path(args.trace_path) if getattr(args, 'trace_path', None) else None)
        config = {
            "client": client,
