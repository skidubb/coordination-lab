...
def main():
    ...
    if args.blackboard:
        ...
        client = make_client(protocol_id="p32_tetlock_forecast", trace=getattr(args, 'trace', False), trace_path=__import__('pathlib').Path(args.trace_path) if getattr(args, 'trace_path', None) else None)
        config = {
            "client": client,
